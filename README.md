...
`same-layer-svg-engine` is the first built-in native geometry patch adapter for that production path. It consumes the latest `svg-patch-plan`, addresses existing source SVG elements by document index, collapses mutable internal partition lines to zero length, and marks them removed in the same layer while preserving original geometry in `data-crab-original-*` attributes. With `--engine-arg=--apply-openings`, it also splits selected wall lines into before/after same-parent segments to create door openings and reports the OpenCrab adjacency edge, connected role, and evidence that selected the opening. It does not add a zoning overlay group, image, or redraw layer.
