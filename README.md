...
`same-layer-svg-engine` is the first built-in native patch adapter for that production path. It consumes the latest `svg-patch-plan`, addresses existing source SVG elements by document index, and changes only those elements' attributes; it does not add a zoning overlay group, image, or redraw layer. This is a deterministic same-layer mutation proof that future trim/move/door-opening solvers can extend.
