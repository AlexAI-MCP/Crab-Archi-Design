...
To test CAD-like line/polyline endpoint grip edits from `same_layer_endpoint_move_candidates`, pass the endpoint-move flag through the engine adapter. Candidate `x/y` and `dx/dy` values are interpreted in recognition/world coordinates; if the source element sits under SVG transforms, the same-layer engine converts the move back into that element's local attributes before writing the SVG.
