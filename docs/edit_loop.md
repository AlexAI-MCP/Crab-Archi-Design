...
The same-layer engine delegates these primitive CAD-like operations to `solver/svg_edit_ops.py`; `solver/svg_mutation.py` only selects patch-plan candidates, enforces locked targets, and reports mutation/QA summaries. The engine summary includes `edit_capability_summary`, which records whether opening splits, endpoint moves, and partition removals are supported or need `review_required` handling for each target primitive. This keeps future wall/path/polyline mutators testable without changing the higher-level OpenCrab/topology handoff.
