...
`mcp-manifest` emits the machine-readable CLI tool catalog for Codex exec, MCP wrappers, OAuth upload workers, and SaaS ingestion services. `mcp-config` emits runtime config for those clients. `crab-archi-design-mcp --stdio` exposes the same catalog through a dependency-free stdio JSON-RPC bridge. See `docs/mcp_oauth_integration.md`.
