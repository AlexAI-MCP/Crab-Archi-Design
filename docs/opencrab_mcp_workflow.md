...
`same-layer-svg-engine` is the preferred built-in adapter for testing the production mutation path. It consumes `svg-patch-plan`, mutates existing source SVG elements by element index, collapses selected mutable internal partition lines to zero length, and records QA gates proving that no overlay layer, redraw layer, raster image, locked-target selection, or locked-geometry mutation was added. It also reports aggregate edit capability totals and QA gates for review-required candidates, so unsupported SVG primitives are visible to the OpenCrab/Codex handoff instead of being masked by redraw output. When run with `--engine-arg=--apply-openings`, it consumes `same_layer_opening_candidates` and splits target wall lines plus open single-subpath M/L/H/V paths into same-parent before/after segments for door openings. Transformed path openings are measured in recognition/world length and converted back into local `d` coordinates before mutation. Those opening candidates are topology-aware: OpenCrab adjacency edges from the target topology graph determine which program boundary is opened first, and the engine report preserves the connected role, cluster id, rationale, and evidence. When run with `--engine-arg=--apply-endpoint-moves`, it consumes `same_layer_endpoint_move_candidates` and grip-edits existing line/polyline endpoints plus open single-subpath M/L/H/V path endpoints by absolute `x/y` or relative `dx/dy` values in recognition/world coordinates, converting through accumulated SVG transforms before touching local element attributes. Curved, arc, and closed paths are skipped for review.

`layout-svg-engine` can be used at the solver step to generate a standards-backed room-envelope candidate without raster overlays. It uses the attached community shell, mutable zone, no-go constraints, recognized column candidates, OpenCrab evidence gate, and standards rows to search for a shell-aware layout box and draw native SVG room partitions for greenery lounge, fitness, golf, wellness, hall, and support programs. It first adds a native cleanup mask over the old mutable/internal layout and then adds a plan-detail layer with partition walls, door openings, a corridor axis, and lounge/hall glazing. The engine report records the repair strategy and keeps the candidate in `review_required` if rooms still escape the community shell, overlap protected no-go envelopes, leave insufficient usable coverage, omit cleanup or plan-detail elements, create inefficient sliver rooms, or lose the intended large-program hierarchy.
