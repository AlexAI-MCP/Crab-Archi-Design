...
`same-layer-svg-engine` is the preferred built-in adapter for testing the production mutation path. It consumes `svg-patch-plan`, mutates existing source SVG elements by element index, and records QA gates proving that no overlay layer, redraw layer, or raster image was added.
